...
def my_handler(scene):
    ...
    # Servo pan
    servo_pan = bpy.data.objects['servopan'].rotation_euler.z
    servo_pan = degrees(servo_pan)
    servo_pan = remap(servo_pan, 90, -90, 75, 225)
    servo_pan = int(servo_pan)

    # Servo tilt
    servo_tilt = bpy.data.objects['servotilt'].rotation_euler.x
    ...
    print("\r" + t1 + str(servo_pan) + s + t2 + str(servo_tilt), end="")
